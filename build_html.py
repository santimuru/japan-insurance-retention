...
body {{ background:#fff; }}
.article {{ max-width: 1000px; margin: 0 auto; padding: var(--space-12) var(--space-8) var(--space-16); }}
/* prose capped at a readable measure and LEFT-aligned to the container edge;
   figures, KPI bands and tables fill the full width. Shared left edge keeps it coherent. */
.article header, .sec > h2, .sec > .sec-meta, .sec > p, .sec > ul.flow, .sec > .tbl-cap, .author {{ max-width: 46rem; }}
...
.sec > table.bi-table {{ max-width: 800px; }}  /* standalone tables: tighten, don't stretch to full width */
/* fairness tables: 5 columns in a half-width figure, so compress to fit (no clip, no wrap) */
#fairAge, #fairInc {{ width: 100%; font-size: .85rem; }}
#fairAge th, #fairAge td, #fairInc th, #fairInc td {{ padding-left: .35rem; padding-right: .35rem; }}
#fairAge td:first-child, #fairInc td:first-child {{ white-space: nowrap; padding-left: 0; }}

/* segment cards: exactly 5 across below the full-width chart (no orphan row) */
.seg-grid {{ display: grid; grid-template-columns: repeat(5, minmax(0, 1fr)); gap: .8rem; margin-top: 1.5rem; }}
...
.seg-card.bi-card {{ background: var(--surface, #fbfcfd); border: 1px solid var(--border); border-radius: 8px;
  box-shadow: none; padding: .85rem .9rem; gap: .35rem; }}
.seg-card h3 {{ font-size: .9rem; font-weight: 600; gap: .4rem; }}
.seg-card .seg-stat {{ font-size: .8rem; }}
.seg-card .note {{ font-size: .84rem !important; line-height: 1.45; color: var(--text-2) !important; }}
